...
session_reboot:
    ...
    if (!g_netplay_pending) {
         ...
         * snes_netplay_apply_env). Same session machinery as a lobby
         * launch, minus the lobby. */
        SnesNetplayConfig env_cfg;
        snes_netplay_config_defaults(&env_cfg);
        snes_netplay_apply_env(&env_cfg);
        if (env_cfg.enabled) {
            g_netplay_cfg = env_cfg;
            g_netplay_pending = 1;
