...
| a single fixed-offset capture of a TYPEWRITER scene | a whole line reads as missing — and the shorter a language's line is, the likelier it lands in the pause after it, so the artifact looks language-specific and therefore like a real bug | poll until the box settles; never trust one frame. Decode the ROM row before believing a data defect: here `es` "line 2 is blank" was a capture in the 0.8 s gap between the two lines |
| a probe that presses A in the same loop it dumps in | the probe dismisses the page it is sampling and reports the blank box as evidence | drive the scene to the surface, then stop sending input and sample only |
| an aggregate patch sharing a row with imported IPS fragments | the row silently stops updating — the fragments do not tile it (an IPS record exists only where the reference differs from the cart), so any partial rewrite leaves a mixture no guard recognises, and a 1-byte gap-fill patch matches by coincidence in every language | give the generator the row OUTRIGHT: split the fragments around it and carry `source` + one payload per language, so a single patch resolves every boot and every switch |
